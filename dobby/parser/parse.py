...
from dobby.classes.endpoint import EndPoint
from dobby.classes.edge import EdgeType, Edge
from dobby.classes.node import NodeType, Node
from dobby.classes.flow import Flow, TCPFlow
from dobby.classes.app import NetworkApp
from dobby.classes.phymodel import PhysicalAddress, PhysicalModel, WifiPhysicalModel
from dobby.classes.metrics import Stats, Metrics, WirelessMetrics, TCPMetrics
from dobby.classes.ipinfo import IPInfo
import json
...
class ParseNodeSummary(object):
    ...
    """
    def __init__(self, fname=None, node_json=None):
        if not fname and not node_json:
            raise IllegalArgumentError("Either json file or string must be input")
        if node_json:
    ...
    def parse_summary(self):
        #Iterate and update the endpoint stats
        for json_node in self.node_json['nodes']['node']:

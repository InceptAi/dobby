...
class Parsenode.NodeSummary(object):
    ...
    def parse_summary(self, node_json, network_summary=None):
        ...
        for json_node in self.node_json['nodes']['node']:
            ...
            if endpoint:
                ...
                endpoint = endpoint.EndPoint(phy_addr)
            ...
            if endpoint.node_id:
                ...
                # Create a node
                node = node.Node(endpoints=[endpoint], node_type=node.NodeType.UNKNOWN)
            ...
            if json_node.get('ip', None):
                ...
                for ip in ips_to_process:
                    ...
                    if node.node_type == node.NodeType.WIRELESS_ROUTER:
                        ...
                        ip_endpoint = endpoint.EndPoint(ip_info = ip_info)
                        ip_node = node.Node(endpoints=[ip_endpoint], node_type=node.NodeType.CLOUD_IP)

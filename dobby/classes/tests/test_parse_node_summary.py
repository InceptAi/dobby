...
class TestParseNodeSummary(unittest.TestCase):
    def setUp(self):
        ...
        for node in self.node_json['nodes']['node']:
            ...
            self.node_count += 1
        self.nodesummary = ParseNodeSummary(node_json=self.node_json)
    ...
    def test_validate_parsing(self):
        ...
        self.assertListEqual(sorted([x.lower() for x in MAC_TO_ENDPOINTS.keys()]), sorted(self.ethers))
        #No cloud IPs, since all are tagged to AP -- albeit incorrectly
        cloud_ip_nodes = [node for node in NODES.values() if node.node_type.value == NodeType.CLOUD_IP.value]
    ...
    def test_validate_ap_is_correctly_tagged(self):
        ...
        MAC_TO_ENDPOINTS[str(phy_addr)] = ap_endpoint
